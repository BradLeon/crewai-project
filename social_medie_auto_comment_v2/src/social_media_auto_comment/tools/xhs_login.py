...
from social_media_auto_comment.config import base_config as config
from social_media_auto_comment.cache.cache_factory import CacheFactory
from xhs_help import convert_cookies, convert_str_cookie_to_dict, find_login_qrcode, show_qrcode
...
class XiaoHongShuLogin():
    ...
    async def login_by_mobile(self):
        ...
        while max_get_sms_code_time > 0:
            logger.info(f"[XiaoHongShuLogin.login_by_mobile] get sms code from redis remaining time {max_get_sms_code_time}s ...")
            await asyncio.sleep(1)
    ...
    async def save_cookies(self):
        ...
        logger.info(f"[XiaoHongShuLogin.save_cookies] Cookies saved to {cookies_file}")
        ...
        try:
            async with aiofiles.open(cookies_file, 'r', encoding='utf-8') as f:
                ...
                if not any(cookie.get('name') == 'web_session' for cookie in cookies):
                    logger.info("[XiaoHongShuLogin.load_saved_cookies] Invalid cookies: web_session not found")
                    return None

                logger.info(f"[XiaoHongShuLogin.load_saved_cookies] Loaded cookies from {cookies_file}")
                return cookies
        except Exception as e:
            logger.error(f"[XiaoHongShuLogin.load_saved_cookies] Error loading cookies: {e}")
            return None
